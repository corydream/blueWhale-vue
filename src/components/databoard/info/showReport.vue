<template>
<div id="show-box" v-if="boardata.boardID" v-loading="loading">
    <div class="nodata" v-if="opacityGraph">
    ...
    :use-css-transforms="true">
        <grid-item  v-for="(v_total,i_total) in getAllTableDatas"  :key="v_total.id"
                    ...
                    @moved="movedEvent">
            <div class="echarts" :id="v_total.style">
                <div class="header-page">
                    <p class="page-title"><b>{{v_total.cname}}</b></p>
                    <p class="page-operate">
                        <!-- <span @click="shareTable(v_total.cid)"><i class="fa fa-share-alt"></i>分享报表</span> -->
                        <span @click="createTable(v_total)"><i class="fa fa-edit"></i>编辑</span>
                        <span @click="delTable(v_total.cid)"><i class="fa fa-trash-o"></i>删除</span>
                    ...
                    <div class="filterControl row" v-if="v_total.chartType == 0">
                        <Row v-for="(confilter, key ,confiler_index) in v_total.conditionMap" :key="confilter.uuid" style="float:left">
                            ...
                            <Col span="24" v-if="confilter.columnType == 'NUMBER'" class="col-confilter">
                                <span>{{confilter.chinese}}:</span>
                                <input type="hidden" :value="confilter.uuid = key">
                                <Select v-model="confilter.relation" style="width:100px" @on-change="changeOpt(confilter)">
                            ...
                            <Col span="24" v-if="confilter.columnType == 'STRING'" class="col-confilter">   
                                ...
                                <Poptip placement="bottom" @on-popper-show="popperLayer(confilter,v_total)">
                                   ...
                                   <div class="api-string" slot="content">
                                       <p class="api-button">
                                            <Input size="small" icon="search" v-model="confilter.search_val" style="width: 200px" @on-change="popperSearch(confilter,v_total)" @on-focus="popperSearch(confilter,v_total)"></Input>
                ...
                <div class="page-table" v-if="v_total.chartType == 0" :class="`page-table_`+v_total.cid">
                    <table class="table table-bordered" :style="{top:v_total.scrollTop +'px'}" style="position:absolute;">
                        <thead>
                            <tr>
                    ...
                    <table class="table table-bordered table-middle table-merge" :id="`merge-table_`+v_total.cid">
                        <tbody >
                            ...
                            <tr v-if="getAllTableDatas[i_total].result.length == 0"><td>没有符合条件的数据，请尝试其他筛选条件</td></tr>
                        </tbody>
                    </table>
                </div>
                <!-- 图表 -->
                <div class="page-echart"  :class="`page-echart_`+i_total">
                    ...
                    <div class="chart-graph chart_1" id="lineChart" :style="{width: '100%', height: '520px'}"></div>
                    <div class="chart-graph chart_2" id="barChart" :style="{width: '100%', height: '520px'}"></div>
                    <div class="chart-graph chart_3" id="pieChart" :style="{width: '100%', height: '520px'}"></div>
                    <div class="chart-graph chart_5" id="radarMapChart" :style="{width: '100%', height: '520px'}"></div>
                </div>
                ...
                <!-- 分页相关 -->
                <div class="pages-bar" v-if="v_total.chartType == 0 &&v_total.result.length>0">
...
export default {
  ...
  created() {
    this.boardata = this.message;
    this.setSessionDatas(this.boardata);
    this.api = this.$keyapi + "/query/download";
  ...
  props: ["message"],
  watch: {
    getRequestDatas: function(val) {
      this.$nextTick(() => {
        this.showGraphs();  
        this.getAllTableDatas.map(v => {
          if(v.chartType == 0){
            document
              .querySelector(`.page-table_${v.cid}`)
              .addEventListener("scroll", function(event) {
                v.scrollTop = event.target.scrollTop;
              });
          }
        });
        this.tablesMerge();
    ...
    "datas.showdrag": function() {
      if (!this.datas.showdrag) {
        let _this = this;
        $.ajax({
          type: "post",
          url: _this.$keyapi + "/auth/report/style",
          dataType: "json",
          xhrFields: {
            withCredentials: true
          },
          data: { reports: JSON.stringify(_this.axisArr) },
          crossDomain: true,
          success: data => {
            if (data.status == 1) {
  ...
  methods: {
    changeOpt(content){ //变更筛选条件中的数字选择时
       content.val = null;
    },
    handleScroll(ev) {
      ev.preventDefault();
      this.getAllTableDatas.map(v => {
        ...
        // if(v.scrollStart){
        //     v.scrollTop = ev.target.scrollTop;
        // }
      });
    },
    //合并单元格
    tablesMerge() {
      this.getAllTableDatas.map(v => {
        if (v.mergeTabs) {
          var listTable = $("#merge-table_" + v.cid);
          var tabelCols = $(`#merge-table_${v.cid} tr:first-child td`);
          var colsAry = [];
    ...
    //取消合并单元格
    untablesMerge(cid) {
      $(`#merge-table_${cid} tbody td`).removeAttr("rowspan style");
    },
    // 分享报表
    shareTable(id) {
      let _this = this;
    ...
    popperCheckAll(item) {
      ...
      if (item.data.length > 200) {
        this.$Message.error("选择的数据超过200条");
    ...
    searchFilter(total, index) {
      let _this = this;
      _this.opacity = true;
      _this.untablesMerge(total.cid);
      let pages = JSON.stringify({ currentPage: 1, pageSize: 20 });
      _this.datas.getDataAllInfos.map((v, i) => {
        if (total.cid == v.reportID) {
          let reportInfo = JSON.parse(v.reportInfo);
          let filtercon = reportInfo.reportInfo.requestObject.sqlParam;
          ...
          for (let obj in total.conditionMap) {
            ...
            if (v_con.columnType == 'NUMBER') {
                ...
                total.conditions.push(
                  {
                    ...
                    val: v_con.val,
                    type: "NUMBER",
                    relation: v_con.relation
                  }
                )
              }
            }
             else if (
              v_con.columnType == "DATE" ||
              v_con.columnType == "DATETIME"
            ) {
              total.conditions.push(
                {
                  uuid: v_con.uuid,
              ...
              v_con.val = [
                v_con.val[0] ? pubMethod.formatDate(v_con.val[0]) : null,
                v_con.val[1] ? pubMethod.formatDate(v_con.val[1]) : null
              ];
    ...
    showNextLevel(item, uuid, total, index) {
      ...
      _this.datas.getDataAllInfos.map((v, i) => {
        if (total.cid == v.reportID) {
          if ((total.drillDowns.length - 1 <= total.conditions.length &&total.chartType >0)   || item == "汇总") {
            return false;
          }
          let reportInfo = JSON.parse(v.reportInfo);
          let filtercon = reportInfo.reportInfo.requestObject.sqlParam;
    ...
    backDrillDown(total, index, item, item_i) {
      ...
      _this.datas.getDataAllInfos.map((v, i) => {
        if (total.cid == v.reportID) {
          ...
          let groupTotal = [];
          filtercon.groupbys = [];
          groupTotal = total.conditions.concat(total.groupbys);
          total.conditions.splice(item_i, allIndex - item_i);
          filtercon.conditions.splice(item_i, allIndex - item_i);
          total.conditions.map((v_con, i_con) => {
            groupTotal.map((v_drill, i_drill) => {
              if (v_con.uuid == v_drill.uuid) {
                groupTotal.splice(i_drill, 1);
              }
            });
          });
          filtercon.groupbys = groupTotal;
          if (item_i == 0) {
            groupConditions = total.groupInitbys;
          } else {
            groupConditions = filtercon.groupbys;
          }
    ...
    drawLine(alldatas, chartDom, isLock, index) {
      ...
      if (
        alldatas.echartConfig.bar.showReverseBar ||
        alldatas.echartConfig.bar.showReverseStackedPlot
      ) {
      ...
      var setPieSeries = {
        ...
        data: zAria,
        itemStyle: {
          normal: { label: { show: alldatas.echartConfig.pie.showTag } }
        },
        labelLine: {
          normal: {
    ...
    clickMyChart(alldatas, params, index) {
      // if(!this.datas.isDrill)return false;
      var uuid = '';
      this.getAllTableDatas.map(v=>{
        if(alldatas.cid == v.cid){
          uuid = v.metas[0].uuid;
          alldatas = v;
        }
      });
      ...
      for (let i = index - loopMessureIndex; i < index; i++) {
        ...
        var obj = {
          ...
          itemStyle: {
            normal: {
              label: {
                show: alldatas.echartConfig[str].showTag,
                position: "top"
              }
            }
          }
        ...
        if (str == "bar") {
          if (
            alldatas.echartConfig.bar.showStackedPlot ||
            alldatas.echartConfig.bar.showReverseStackedPlot
          ) {
            obj.stack = "总量";
    ...
    delTable(id) {
      // 删除接口
      let _this = this;
      _this.datas.delTarget = true;
      let ids = _this.$layer.confirm(
        "确定要删除吗？",
        function() {
          $.ajax({
            type: "post",
    ...
    exportTable(v) {
      var sqlparamStr = {
        columnMessure: v.columnMessure,
        groupbys: v.groupbys,
        conditions: v.conditions,
        project: JSON.parse(v.reportObj).project
      };
      // location.href = this.$keyapi+`/query/download?cid=${v.cid}`;
      // location.href = `http://192.168.38.103:8610/v/1.mp4`;
      // try{
      ...
      $.ajax({
        ...
        success: function(res) {
          if (res.status == 1) {
            ...
            _this.datas.getDataOriginalInfos = pubMethod.objClone(
              _this.datas.getDataAllInfos
            );
          } else if (res.status == 101) {
            location.href = "https://data.banggood.cn";
    ...
    requestTabelDatas(requestDatas, pages, index, searchSql, pageMark) {
      ...
      // 优化数据查询
      for (let optimize1 in dataObj.reportInfo.requestObject.sqlParam
        .drillDowns) {
        dataObj.reportInfo.requestObject.sqlParam.drillDowns[
          optimize1
        ].listData = [];
      }
      for (let optimize2 in dataObj.reportInfo.requestObject.sqlParam
        .conditionMap) {
        dataObj.reportInfo.requestObject.sqlParam.conditionMap[
          optimize2
        ].data = [];
      }
      ...
      let reportObj = {
        ...
        CompareColumn: dataObj.reportInfo.requestObject.sqlParam.CompareColumn,
        fromBegin: dataObj.reportInfo.requestObject.sqlParam.fromBegin,
        fromEnd: dataObj.reportInfo.requestObject.sqlParam.fromEnd,
        compareBegin: dataObj.reportInfo.requestObject.sqlParam.compareBegin,
        compareEnd: dataObj.reportInfo.requestObject.sqlParam.compareEnd,
        compareName: []
      };
      dataObj.reportInfo.requestObject.sqlParam.columnMessure.map(v => {
        if (v.isCompare) {
          reportObj.compareName.push(v.chinese);
        }
      });
      _this.datas.editListPages = dataObj.reportInfo.editListPages;
      _this.datas.editListPages.pageSize = 20;
      if (!pages) {
      ...
      $.ajax({
        ...
        success: function(res) {
          if (res.status == 1) {
            ...
            res.data.drillDowns =
              dataObj.reportInfo.requestObject.sqlParam.drillDowns;
            res.data.groupInitbys =
              dataObj.reportInfo.requestObject.sqlParam.groupInitbys;

            res.data.sqlparamStr = JSON.stringify({
              columnMessure:
                dataObj.reportInfo.requestObject.sqlParam.columnMessure,
              ...
              CompareColumn: dataObj.reportInfo.requestObject.sqlParam
                .CompareColumn
                ? dataObj.reportInfo.requestObject.sqlParam.CompareColumn
                : {},
              compareBegin:
                dataObj.reportInfo.requestObject.sqlParam.compareBegin,
              compareEnd: dataObj.reportInfo.requestObject.sqlParam.compareEnd,
              fromBegin: dataObj.reportInfo.requestObject.sqlParam.fromBegin,
              fromEnd: dataObj.reportInfo.requestObject.sqlParam.fromEnd
            });
            res.data.echartConfig =
              dataObj.reportInfo.requestObject.sqlParam.echartConfig;
            res.data.groupbys =
              dataObj.reportInfo.requestObject.sqlParam.groupbys;
            res.data.columnMessure =
              dataObj.reportInfo.requestObject.sqlParam.columnMessure;
            res.data.mergeTabs =
              dataObj.reportInfo.requestObject.sqlParam.mergeTabs;
            res.data.scrollTop = 0;
            let maplen = Math.ceil(
              Object.getOwnPropertyNames(res.data.conditionMap).length / 4
            );
            res.data.t_bottom = 412 - (maplen - 1) * 40 + "px";
            if (typeof index == "number") {
              if (JSON.parse(res.data.reportObj).styleType) {
                let styleArr = JSON.parse(
                  JSON.parse(res.data.reportObj).styleType
                );
              ...
              if (_this.pageTarget) {
                res.data.conditions =
                  dataObj.reportInfo.requestObject.sqlParam.conditions;
              }
            ...
            if (_this.dataFlagLength == _this.getAllTableDatas.length) {
      ...
      for (var i = 0; i < this.getAllTableDatas.length; i++) {
        ...
        if (this.getAllTableDatas[i].chartType > 0) {
          ...
          if (this.getAllTableDatas[i].chartType > 4) {
            $(".page-echart_" + i)
              .children(".chart-graph")
              .eq(this.getAllTableDatas[i].chartType - 2)
              .show();
          } else if (this.getAllTableDatas[i].chartType < 4) {
            $(".page-echart_" + i)
              .children(".chart-graph")
              .eq(this.getAllTableDatas[i].chartType - 1)
              .show();
          }
        }
        // if (this.datas.delTarget &&this.getAllTableDatas[i].chartType>0){
          
        // }
          this.drawLine(
            this.getAllTableDatas[i],
            $(".page-echart_" + i)[0],
            true,
            i
          );
    ...
    createTable(obj) {
      ...
      $.ajax({
        type: "post",
        url: _this.$keyapi + "/auth/check/edit/report",
        xhrFields: {
          withCredentials: true
        },
        data: {
        ...
        success: function(res) {
          if (res.status == 1) {
            _this.loading = false;
            _this.boardata.project = JSON.parse(obj.reportObj).project;
            _this.boardata.echoPage = true;
            _this.boardata.cid = obj.cid;
            _this.boardata.cinfo = JSON.parse(obj.reportObj).cinfo;
            _this.boardata.modelID = obj.modelID;
            _this.boardata.modelName = obj.modelName;
            _this.boardata.chartType = obj.chartType ? obj.chartType : 0;
            _this.boardata.tableObj = obj;
            _this.$layer.iframe({
              content: {
    ...
    exportData(downloadTable) {
      pubMethod.exportExcel("downloadTable");
    },
    // nextpage
    mPageSize(total, index) {
      ...
      _this.datas.getDataAllInfos.map((v, i) => {
        if (total.cid == v.reportID) {
          ...
          _this.requestTabelDatas(
            v,
            JSON.stringify(total.pageBean),
            index,
            null,
            true
          );
    ...
    resizeCharts(element, w, h) {
      ...
      // console.log("设置之后" + element.clientWidth + " " + element.clientHeight + " " + w + " " + h);
      bus.$emit("echart", element);
    },
    ...
    resizedEvent(i, newH, newW, newHPx, newWPx) {
      ...
      //获取图表外层的盒子
      let chart = document.getElementById("my" + i);
      //改变图表外层的宽高
      this.resizeCharts(chart, newWPx, newHPx);
    }
  }
};
</script>
