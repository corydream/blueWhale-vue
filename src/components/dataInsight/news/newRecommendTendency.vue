    ...
    <div>
      <div class="wrap row">
        <CategoryItem
          ...
          :chooseType="chooseType"v-if="modelShow.categoryDisplay" ref="category"
        >
        </CategoryItem>
      </div>
      <TableList :TableItem="TableItem" :showMessage="showMessage"
                 :chooseType="chooseType" v-if="modelShow.tableDisplay"
                 ref="table">
      </TableList>
    </div>
  </div>
...
<script>
  ...
  //页面初始数据
  import initDatas from "../../../config/insight/recommendDatas/amazon/recommendData3";
  //通用方法
  ...
  export default {
    data() {
      return {
        ...
        categoryItem:{
          ...
          parmasConfig:initDatas.parmasConfig,
          initCondition:initDatas.initCondition,
          categoryTitle:"Hot New Release数据",
          type:"news",
          showNewColumn:"name3",
    ...
    mounted() {
     /* this.$nextTick(() => {
        ...
        window.onresize = () => {
          //重置容器高宽
          if (this.$route.path == "/dataObserve/0/news/newRecommend/newRecommendTendency") {
            //自适应表格
            commonUtil.setTableWidth({
              wrapper:".data-show",
              tables:this.TableItem.tableID,
