<template>
    <div>
        <div class="statleList">
          <p>
            <span class="statleList-title">稳定周期:</span>
            <RadioGroup v-model="currentCycle" @on-change="changeCycle">
              <Radio label="7天"></Radio>
              <Radio label="15天"></Radio>
              <Radio label="30天"></Radio>
            </RadioGroup>
          </p>
        </div>
   <!--   <div id="scrollTable">
...
<script>
  ...
  //表格相关类
  import  tableUtil from "../../../../../../config/insight/commons/tablesListClass2"
 ...
 /* import IScroll from "iscroll";*/
  export default {
        ...
        /*    this.scrolObj = new IScroll('#scrollTable', {
            ...
            this.changeSelect(this.collectBtnsActive||"tab2");
        ...
        methods:{
          changeCycle(newDay){
             newDay=newDay.substr(0,newDay.length-1);
             tableUtil.changeDays(newDay);
             tableUtil.changeCurrentCycle();
          },
          ...
          sortAll(item) {
            ...
            this.TableItem.initCondition.measures.forEach((mes,index)=>{
              if(mes.key===item.key){
                ...
                this.$set(condition,"key",mes.key);
                this.$set(condition,"sortType",mes.sortType);
                this.$set(condition,"relation",mes.relation);
                this.$set(condition,"chinese",mes.chinese);
              }else{
                this.$set(mes, "sortType", "DEFAULT");
              }
            })
            if(this.TableItem.initCondition.condition.length>0){
              let flagSort=false;
              this.TableItem.initCondition.condition.forEach((row,i)=>{
                if(row.key==condition.key){
                  //已经有排序
                  flagSort=true;
                  this.$set(row,'sortType',condition.sortType);
                }else{
                  if((row.val&&row.val!="")
                    ||(row.minVal&&row.minVal!="")
                    ||(row.maxVal&&row.maxVal!="")){
                    this.$set(row,'sortType',"DEFAULT");
                  }else{
                    //删除该对象
                    this.TableItem.initCondition.condition.splice(i,1);
                  }
                }
              })
              if(!flagSort){
                this.TableItem.initCondition.condition.push(condition);
              }
            }else{
              this.TableItem.initCondition.condition.push(condition);
            }
