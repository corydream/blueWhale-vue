...
export default {
  ...
  watch: {
    "TableItem.tableContainer.productData": function(val) {
        this.currentData.sortData.splice(0,this.currentData.sortData.length);
        val.forEach((item,i)=>{
          this.currentData.sortData.push({
            label:item.label,
            value:item.value,
            isSelected:item.value==this.TableItem.tableContainer.queryData.product.inputVal?true:false
          });
        })
    },
    "TableItem.tableContainer.islock":function (val) {this.noMoreData=val;},
    "TableItem.tableContainer.queryData.product.inputVal":function (val) {
      if(this.TableItem.tableContainer.productData.length>0){
        this.TableItem.tableContainer.productData.forEach((item)=>{
          if(item.value==val){
  ...
  methods: {
    ...
    //选中切换
     changeSelected(item) {
      ...
      this.$set(this.TableItem.tableContainer.queryData.productData,"inputVal",this.currentData.inputVal);
    },
    //初始化监听滚动事件
     initScroll(){
       if (!this.isFirst) {
    ...
    //箭头滚动加载更多数据
     handelScroll(e){
      ...
      if(scrollTop + wrapHeight >= scrollHeight){
        ...
        this.$parent.getBottomProductData();
      }
    },
     queryBsr(){
       ...
       this.arrowStyle = this.arrowState ? "arrow-down-b" : "arrow-up-b";
       //调用搜索
       this.$parent.getSumProductName(this.currentData.inputVal);
     }
  }
};
</script>
